(() => {
  ...
  function replace(code, label, pattern, replacement) {
    if (code.includes("hostileTimer") && label !== "blood sense hostile marker") return code;
    const next = code.replace(pattern, replacement);
    if (next === code) console.warn(`[VD loader] Patch skipped: ${label}`);
    else console.info(`[VD loader] Patch applied: ${label}`);
    return next;
  }

  function insertAfter(code, label, marker, insertion) {
    if (code.includes("function nearestHostileCivilian")) return code;
    const idx = code.indexOf(marker);
    if (idx < 0) {
      console.warn(`[VD loader] Patch skipped: ${label}`);
      return code;
    }
    console.info(`[VD loader] Patch applied: ${label}`);
    const end = idx + marker.length;
    return code.slice(0, end) + insertion + code.slice(end);
  }

  function patchStreetBrawl(code) {
    code = replace(
      code,
      "NPC hostile fields",
      /alertTimer: 0,\n\s*dragged: false,/,
...
`
    ...
    code = replace(code, "handle action can shove hostile pedestrian", /      const alarmed = nearestAlarmedWitness\(\);\n      if \(alarmed\) \{\n        interceptWitness\(alarmed\);\n        return;\n      \}\n\n      const victim = nearestFeedable\(\);/, `      const alarmed = nearestAlarmedWitness();
      if (alarmed) {
        interceptWitness(alarmed);
        return;
      }
    ...
    code = replace(code, "hostile civilian update priority", /      if \(updateAlarmedWitness\(n, dt\)\) return;\n      if \(n\.fleeTimer > 0\) n\.fleeTimer -= dt;/, `      if (updateAlarmedWitness(n, dt)) return;
      if (updateHostileCivilian(n, dt)) return;
      if (n.fleeTimer > 0) n.fleeTimer -= dt;`);

    code = replace(code, "lamp break can provoke pedestrian", /      const witnesses = visibleWitnessList\(145\)\.filter\(w => w\.type !== "hunter"\);\n      addLocalHeat/, `      const witnesses = visibleWitnessList(145).filter(w => w.type !== "hunter");
      provokeStreetBrawlers(light.x, light.y, witnesses);
      addLocalHeat`);

    code = replace(code, "context prompt hostile", /      const victim = nearestFeedable\(\);\n      if \(victim\) \{/, `      const hostile = nearestHostileCivilian();
      if (hostile) return "E: shove angry pedestrian away";
      const victim = nearestFeedable();
      if (victim) {`);

    code = replace(code, "draw hostile color", /        if \(n\.luredTimer > 0\) c = "#d7c8ff";\n/, `        if (n.luredTimer > 0) c = "#d7c8ff";
        if (n.hostileTimer > 0) c = "#ff5c35";
`);

    code = replace(code, "draw hostile marker", /        if \(n\.alarmed && !n\.hasReported\) \{\n          ctx\.fillStyle = "#ff3b50";\n          ctx\.fillRect\(Math\.floor\(n\.x - 1\), Math\.floor\(n\.y - 16\), 2, 7\);\n          ctx\.fillRect\(Math\.floor\(n\.x - 1\), Math\.floor\(n\.y - 7\), 2, 2\);\n        \} else if \(n\.luredTimer > 0\) \{/, `        if (n.hostileTimer > 0) {
    ...
    code = replace(code, "blood sense hostile marker", /        else if \(n\.alarmed && !n\.hasReported && d < 300\) drawSenseMarker\(n\.x, n\.y, "witness", "#ff3b50", true\);\n/, `        else if (n.hostileTimer > 0 && d < 300) drawSenseMarker(n.x, n.y, "angry", "#ff5c35", true);
...
`);
  ...
  xhr.open("GET", "js/game.js", false);
  xhr.send(null);
  if (xhr.status < 200 || xhr.status >= 300) {
    throw new Error(`Could not load js/game.js: ${xhr.status}`);
  }

  const script = document.createElement("script");
  script.textContent = `${patchStreetBrawl(xhr.responseText)}\n//# sourceURL=js/game.js`;
