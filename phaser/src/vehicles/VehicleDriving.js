...
export function handleVehicleWorldCollision(system, vehicle, impactSpeed) {
  ...
  if (impact >= 36 && system.crashCooldown <= 0) {
    ...
    system.scene.exposureSystem?.add?.(
      Math.min(7, Math.max(2, Math.ceil(impact / 38))),
      `${vehicle.name} crashes into the streetscape. The impact carries through the district.`
    );
...
export function updateVehicleDriving(system, dt, frame) {
  ...
  if (canVehicleOccupy(system, vehicle, next.x, next.y, next.angle)) {
