...
const CDN_PHASER_SOURCES = Object.freeze([
  Object.freeze({ kind: "jsdelivr", src: `https://cdn.jsdelivr.net/npm/phaser@${PHASER_VERSION}/dist/phaser.min.js` }),
  Object.freeze({ kind: "unpkg", src: `https://unpkg.com/phaser@${PHASER_VERSION}/dist/phaser.min.js` })
]);
...
function phaserScriptSources() {
  return localPhaserAllowed() ? [LOCAL_PHASER_SOURCE, ...CDN_PHASER_SOURCES] : CDN_PHASER_SOURCES;
}
...
function loadScript(source) {
  return new Promise((resolve, reject) => {
    ...
    if (existing) {
      if (window.Phaser) { resolve(source); return; }
...
async function ensurePhaser() {
  if (window.Phaser) return publishPhaserSource({ kind: "existing", version: window.Phaser.VERSION || "unknown" });
  let lastError = null;
  for (const source of phaserScriptSources()) {
    try {
      await loadScript(source);
      if (window.Phaser) return publishPhaserSource({ kind: source.kind, src: source.src, version: window.Phaser.VERSION || PHASER_VERSION });
...
function installPlaytestIntroPolicy(UIScene) {
  ...
  Object.defineProperty(prototype, "__nbdPlaytestIntroPolicy", { value: true, configurable: true });
}
...
function renderBootFailure(error) {
  ...
  if (!root) return;
  root.innerHTML = `<div style="display:grid;place-items:center;min-height:320px;padding:32px;text-align:center;background:#090a12;color:#f4ecff;border:1px solid #513c65"><div><strong style="display:block;margin-bottom:10px;color:#ffb02e">Viceblood could not start</strong><span style="font-size:13px;line-height:1.5;color:#c9bfd7">${String(error?.message || error || "Unknown boot error")}</span></div></div>`;
}
...
try {
  ...
  if (bootProfile.mode === BOOT_MODES.PLAYTEST) await import(`./playtest/bootstrap.js?v=${PLAYTEST_ASSET_VERSION}`);
  ...
  window.dispatchEvent(new CustomEvent("nbd:app-ready", { detail: {
    phaser, campaign: true, registeredMissions: 0, rcTest: bootProfile.rcTest, bootProfile,
