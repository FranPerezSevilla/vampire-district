...
function drawOutskirts(scene) {
  ...
  // Existing avenues continue beyond the playable district.
  ...
  // Additional streets make the district read as one neighbourhood inside a larger city.
  ...
  // Static street details keep the outskirts from reading as a flat backdrop.
  ...
  // Subtle playable-area border and exit chevrons.
...
function applyOutskirtsVisibility(scene) {
  if (!scene.outskirtsGraphics) return;
  const alpha = scene.currentLayer === LAYERS.STREET
    ? 1
    : scene.currentLayer === LAYERS.SEWER
      ? 0.16
      : 0.52;
  scene.outskirtsGraphics.setAlpha(alpha);
}

function restoreExpandedCameraBounds(scene) {
  if (!scene?.cameras?.main) return;
  if (scene.registry.get("taskRevealActive")) return;
  if (scene.tutorialDirector?.busy) return;
...
async function showBoundaryWarning(scene) {
  ...
  if (scene.__nbdBoundaryWarningActive) return;
  if ((scene.__nbdBoundaryWarningUntil || 0) > scene.time.now) return;
  if (director.busy || director.state !== "complete") return;
  if (scene.missionSystem?.failed || scene.missionSystem?.completed) return;

  scene.__nbdBoundaryWarningActive = true;
  director.busy = true;
...
function installBoundaryMovementWarning() {
  ...
  GameScene.prototype.__nbdDistrictBoundaryPatch = true;
}

function attachOutskirts() {
  const scene = window.NBD_PHASER_GAME?.scene?.getScene?.("GameScene");
  if (!scene?.player || !scene?.cameras?.main || !scene?.tutorialDirector) {
    window.requestAnimationFrame(attachOutskirts);
    return;
  }
  if (scene.__nbdOutskirtsInstalled) return;

  scene.outskirtsGraphics = drawOutskirts(scene);
  restoreExpandedCameraBounds(scene);
  applyOutskirtsVisibility(scene);
  ...
  scene.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
    scene.outskirtsGraphics?.destroy();
    scene.outskirtsGraphics = null;
  });

  scene.__nbdOutskirtsInstalled = true;
}

installBoundaryMovementWarning();
attachOutskirts();
