...
function objectivePoint(x, y, layer, color) {
...
class PlayerObjectiveArrow {
  ...
  currentObjective() {
    ...
    if ((mission.rooftopJumps || 0) < 1) {
      return objectivePoint(FIRST_JUMP.ax, FIRST_JUMP.ay, FIRST_JUMP.aLayer, ROUTE_COLOR);
    }

    const thug = liveNpc(this.scene, "rooftop_thug");
    if (thug && !thug.dead) {
      return objectivePoint(thug.x, thug.y, thug.layer, DANGER_COLOR);
    }

    return objectivePoint(775, 150, LAYERS.ROOF_LOW, TIP_COLOR);
  }
