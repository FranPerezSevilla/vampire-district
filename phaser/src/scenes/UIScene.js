...
export class UIScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.phase = this.add.text(90, 612, "PHASE 14: vampire veil rooftop blocker", {
      fontFamily: FONT,
      fontSize: "10px",
      color: "#ffb02e",
  ...
  createTopHud() {
    ...
    this.title = this.add.text(16, 10, "Night Blood District", { fontFamily: FONT, fontSize: "14px", color: "#f1e6ff" }).setScrollFactor(0).setDepth(11);
    this.hungerBadge = this.add.text(310, 10, "Hunger --", { fontFamily: FONT, fontSize: "12px", color: "#ff3b50", backgroundColor: "rgba(53,16,27,.72)", padding: { x: 8, y: 5 } }).setScrollFactor(0).setDepth(11);
    this.exposureBadge = this.add.text(474, 10, "Exposure --", { fontFamily: FONT, fontSize: "12px", color: "#ffb02e", backgroundColor: "rgba(45,29,10,.72)", padding: { x: 8, y: 5 } }).setScrollFactor(0).setDepth(11);
    ...
    this.menuButtonLabel = this.add.text(854, 15, "MENU · H", { fontFamily: FONT, fontSize: "11px", color: "#78c7a3" }).setScrollFactor(0).setDepth(13);
  ...
  createPowerOrbs() {
    ...
    this.powerRailTitle = this.add.text(17, 100, "POWERS", { fontFamily: FONT, fontSize: "9px", color: "#9d93b8", letterSpacing: 1 }).setScrollFactor(0).setDepth(17);
    for (const [id, cfg] of Object.entries(POWER_CONFIG)) {
      this.powerTexts[id] = {
        key: this.add.text(cfg.x, cfg.y - 2, cfg.key, { fontFamily: FONT, fontSize: "13px", color: "#f1e6ff" }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(17),
        label: this.add.text(cfg.x, cfg.y + 27, cfg.label, { fontFamily: FONT, fontSize: "9px", color: "#d7c8ff" }).setOrigin(0.5, 0.5).setScrollFactor(0).setDepth(17)
      };
    }
  }

  createFooter() {
    this.objective = this.add.text(90, 520, "Objective: booting...", { fontFamily: FONT, fontSize: "11px", color: "#ffb02e", backgroundColor: "rgba(0,0,0,.55)", padding: { x: 8, y: 4 }, wordWrap: { width: 760 } }).setScrollFactor(0).setDepth(15);
    this.prompt = this.add.text(90, 550, "", { fontFamily: FONT, fontSize: "10px", color: "#fff2a8", backgroundColor: "rgba(0,0,0,.50)", padding: { x: 8, y: 4 }, wordWrap: { width: 760 } }).setScrollFactor(0).setDepth(15);
    this.lastAction = this.add.text(90, 586, "", { fontFamily: FONT, fontSize: "9px", color: "#9d93b8", backgroundColor: "rgba(0,0,0,.45)", padding: { x: 8, y: 4 }, wordWrap: { width: 760 } }).setScrollFactor(0).setDepth(15);
  }
  ...
  createInteractionMenu() {
    ...
    this.menuTitle = this.add.text(280, 224, "Choose interaction", { fontFamily: FONT, fontSize: "14px", color: "#f1e6ff" }).setScrollFactor(0).setDepth(102).setVisible(false);
    this.menuHint = this.add.text(280, 244, "W/S or arrows · E/Enter confirm · Esc cancel · 1-9 quick select", { fontFamily: FONT, fontSize: "9px", color: "#9d93b8" }).setScrollFactor(0).setDepth(102).setVisible(false);
    this.menuRows = [];
    for (let i = 0; i < 9; i++) {
      const y = 270 + i * 18;
      const bg = this.add.rectangle(480, y - 4, 410, 16, 0x78c7a3, 0.0).setScrollFactor(0).setDepth(102).setVisible(false);
      const label = this.add.text(286, y - 10, "", { fontFamily: FONT, fontSize: "10px", color: "#f1e6ff" }).setScrollFactor(0).setDepth(103).setVisible(false);
      const detail = this.add.text(560, y - 10, "", { fontFamily: FONT, fontSize: "9px", color: "#9d93b8" }).setScrollFactor(0).setDepth(103).setVisible(false);
  ...
  createIntroOverlay() {
    ...
    add(this.add.text(222, 144, "Night Blood District", { fontFamily: FONT, fontSize: "26px", color: "#f1e6ff" }).setScrollFactor(0).setDepth(222));
    add(this.add.text(222, 184,
      "You are a vampire fixer for the clan. A journalist is close to exposing what hunts this district. If civilians understand the truth, the veil breaks and the run is over.",
      { fontFamily: FONT, fontSize: "13px", color: "#d7c8ff", lineSpacing: 7, wordWrap: { width: 540 } }
    ).setScrollFactor(0).setDepth(222));
    add(this.add.text(222, 278,
      "First task: move across rooftops, neutralize the thug blocking the jump to the police station roof, and collect the informant tip. Then find the journalist near the nightclub, isolate them, and clean the scene.",
      { fontFamily: FONT, fontSize: "12px", color: "#78c7a3", lineSpacing: 6, wordWrap: { width: 540 } }
    ).setScrollFactor(0).setDepth(222));
    add(this.add.text(222, 366,
      "Controls: WASD/arrows move · Shift sprint · E interact · Q/Space Dash · R Whisper · F Blood Sense · H Menu",
      { fontFamily: FONT, fontSize: "11px", color: "#ffb02e", lineSpacing: 6, wordWrap: { width: 540 } }
    ).setScrollFactor(0).setDepth(222));
    ...
    add(this.add.text(414, 435, "Start run · Enter", { fontFamily: FONT, fontSize: "13px", color: "#ffb02e" }).setScrollFactor(0).setDepth(223));
  ...
  createPauseOverlay() {
    ...
    add(this.add.text(174, 132, "Pause Menu", { fontFamily: FONT, fontSize: "20px", color: "#f1e6ff" }).setScrollFactor(0).setDepth(242));
    ...
    this.controlsTabText = add(this.add.text(222, 168, "Controls", { fontFamily: FONT, fontSize: "12px", color: "#78c7a3" }).setScrollFactor(0).setDepth(243));
    this.statsTabText = add(this.add.text(374, 168, "Stats", { fontFamily: FONT, fontSize: "12px", color: "#9d93b8" }).setScrollFactor(0).setDepth(243));
    this.controlsTab.on("pointerdown", () => this.setPauseTab("controls"));
    this.statsTab.on("pointerdown", () => this.setPauseTab("stats"));
    this.pauseContent = add(this.add.text(176, 214, "", { fontFamily: FONT, fontSize: "12px", color: "#d7c8ff", lineSpacing: 7, wordWrap: { width: 610 } }).setScrollFactor(0).setDepth(242));
    ...
    add(this.add.text(426, 486, "Close · H / Esc", { fontFamily: FONT, fontSize: "12px", color: "#78c7a3" }).setScrollFactor(0).setDepth(243));
  ...
  createResultOverlay() {
    ...
    this.resultTitle = add(this.add.text(190, 134, "Mission Result", { fontFamily: FONT, fontSize: "22px", color: "#f1e6ff" }).setScrollFactor(0).setDepth(302));
    this.resultSubtitle = add(this.add.text(190, 166, "", { fontFamily: FONT, fontSize: "12px", color: "#9d93b8", wordWrap: { width: 580 } }).setScrollFactor(0).setDepth(302));
    this.resultStats = add(this.add.text(190, 214, "", { fontFamily: FONT, fontSize: "12px", color: "#d7c8ff", lineSpacing: 7, wordWrap: { width: 580 } }).setScrollFactor(0).setDepth(302));
    this.resultHint = add(this.add.text(190, 492, "", { fontFamily: FONT, fontSize: "11px", color: "#78c7a3" }).setScrollFactor(0).setDepth(302));
  }
  ...
  update() {
    ...
    const prompt = menu ? "Interaction menu open" : this.registry.get("interactionPrompt") || "Q/Space Dash · R Whisper · F Blood Sense · E interact/feed/body/routes";
