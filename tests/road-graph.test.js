...
test("the full city is reproducible from its explicit road graph with no road-piece overlap", () => {
  ...
  assert.equal(compiled.stats.sidewalkCount, 772);
  assert.equal(compiled.stats.roadEdgeBandCount, 288);
  assert.equal(compiled.stats.roadEdgeBandSourceCount, 288);
  assert.equal(compiled.stats.absorbedShortApproachCount, 0);
  assert.equal(compiled.stats.junctionSidewalkCount, junctionSidewalks.length);
  assert.equal(compiled.stats.crosswalkCount, 141);
  assert.equal(compiled.stats.propExclusionZoneCount, 540);
  assert.equal(compiled.stats.propExclusionZoneCount, propExclusionZones.length);
  assert.equal(compiled.stats.lightCount, 86);
