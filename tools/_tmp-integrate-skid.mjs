...
docs = docs.replace("- `vehicleSkidLoop` — currently procedural and gameplay-wired to aggressive driving; replace with a real tyre-skid loop next", "- `vehicleSkidLoop` — **integrated candidate:** real gap-sensitive PCM loop sustained while aggressive drifting continues; pending listening acceptance");
docs = docs.replace("Given the newly gameplay-wired aggressive-driving panic, source `vehicleSkidLoop` next, then `ambienceStreetNight`.", "With `policeSirenLoop` and `vehicleSkidLoop` now wired as listening candidates, continue sourcing with `ambienceStreetNight`.");
write(docsPath, docs);

for (const path of [".github/workflows/integrate-vehicle-skid-audio.yml", "tools/_tmp-integrate-skid.mjs"]) {
