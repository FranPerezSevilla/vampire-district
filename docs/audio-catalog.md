...
**Durable production procedure:** read [`AUDIO_ASSET_PIPELINE.md`](AUDIO_ASSET_PIPELINE.md) before integrating any sourced audio. The ordered police/combat/death follow-up is tracked in [`PLAYTEST_ESCALATION_DAMAGE_RECOVERY.md`](PLAYTEST_ESCALATION_DAMAGE_RECOVERY.md). That document defines the human/assistant handoff, binary-upload rules, variant policy, attribution requirements and recovery procedure between sessions.
