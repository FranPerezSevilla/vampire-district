...
export function MapCanvas({ geometry, model, selection, onSelect, onMark }) {
  ...
  return <div className="vb-map-column">
    <div className="vb-map-tools" aria-label="Map layers"><span>STREET ATLAS / INK LAYER</span>{[["hunting", "Hunting rights"], ["owner", "Who rules"], ["relationship", "Reception"]].map(([id, label]) => <Button key={id} aria-pressed={layer === id} onClick={() => setLayer(id)}>{label}</Button>)}</div>
    ...
    <div className="vb-map-legend">{layer === "owner" ? <><span><i style={{ background: OWNER_COLORS.first_estate }}/>First Estate</span><span><i style={{ background: OWNER_COLORS.gutter_crown }}/>Gutter Crown</span><span><i style={{ background: OWNER_COLORS.independent }}/>Independent / contested</span></> : layer === "hunting" ? <><span><i style={{ background: "#6caa8b" }}/>General permit held</span><span>No permit ≠ hostile</span></> : <><span>Colour reflects your relationship with the territorial owner.</span><span>Not your hunting permission.</span></>}<span>Drag to pan · double-click to save a waypoint</span></div><div className="nb-map-key" aria-label="Location symbols"><span><b>●</b> contact</span><span><b>♦</b> donor</span><span><b>▣</b> business</span><span><b>×</b> task / marker</span></div>
  </div>;
}
export function City({ model, geometry, selection, command }) {
  ...
  return <div className="vb-city-layout"><MapCanvas geometry={geometry} model={model} selection={selection} onSelect={target => command("select", { target })} onMark={point => command("save-marker", { point })}/><aside className="vb-city-detail">
    <label className="nb-district-picker"><span className="vb-eyebrow">CHAPTER 02 / INSPECT A DISTRICT</span><select aria-label="Inspect a district" value={selectedDistrict?.id || ""} onChange={event=>command("select",{target:`district:${event.target.value}`})}>{model.districts.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select></label>
    {point ? <div className="nb-map-file"><span className="vb-eyebrow">{point.kind} / SELECTED RECORD</span><h3>{point.name}</h3><p>{point.role || point.description || point.patron && `Patron: ${point.patron}` || "Saved location"}</p><Badge tone={point.suspended || point.refused ? "danger" : "neutral"}>{point.status || (point.level ? "Business interest" : "Location")}</Badge><p>{point.benefits || point.reason || point.requirement}</p>
      <Locations target={point.target} command={command} disabled={point.dead}/>{["contact","donor","asset"].includes(point.kind) && <Button onClick={()=>command("tab",recommendationFile(point.target))}>Open full file</Button>}{point.kind !== "marker" && <Button onClick={() => command("save-marker", { target: point.target })}>Save location</Button>}{point.kind === "marker" && <Button danger onClick={() => command("remove-marker", { id: point.id })}>Remove marker</Button>}<hr/></div> : null}
    {selectedDistrict && <><span className="vb-eyebrow">TERRITORIAL RECORD</span><h3>{selectedDistrict.name}</h3><div className="nb-permit" data-permitted={selectedDistrict.permitted}>{selectedDistrict.permitted ? "HUNTING / GENERAL PERMIT HELD" : "HUNTING / NO GENERAL PERMIT"}</div><p>Not a promise of safety. Protected prey and personal consent have separate rules.</p><dl className="vb-facts"><div><dt>Who rules here</dt><dd>{selectedDistrict.ownerLabel}<small>{selectedDistrict.politicalStatus}</small></dd></div><div><dt>How they receive you</dt><dd>{relationshipLabel(selectedDistrict.relationship)}{selectedDistrict.reputation != null && <small>Reputation {selectedDistrict.reputation}</small>}</dd></div></dl>
      <h4>Contacts · power & favours</h4>{local(contacts).map(p => <button type="button" key={p.id} className="vb-location-row" onClick={() => command("tab", { tab:"network", target: p.target })}><strong>{p.name} ↗</strong><span>{p.status} · Open file</span></button>)}{!local(contacts).length && <p>No network contacts recorded here.</p>}
      <h4>Donors · personal agreements</h4>{local(herd).map(p => <button type="button" key={p.id} className="vb-location-row" onClick={() => command("tab", { tab:"feeding", target: p.target })}><strong>{p.name} ↗</strong><span>{p.status} · Open feeding record</span></button>)}{!local(herd).length && <p>No donor agreements recorded here.</p>}
      <h4>Business interests</h4>{local(assets).map(p => <button type="button" key={p.id} className="vb-location-row" onClick={() => command("tab", { tab:"ledger", target: p.target })}><strong>{p.name} ↗</strong><span>{p.suspended ? "Suspended" : ["Not owned", "Investor", "Controlled"][p.level]}</span></button>)}{!local(assets).length && <p>No network businesses recorded here.</p>}</>}
  </aside></div>;
}
