...
function Hud({ s, command }) {
  ...
  return <div className="vb-hud" data-viceblood-ui="hud">
    ...
    <div className="vb-hud-nav"><Button icon="city" onClick={() => command("open", { tab: "city" })}>City</Button><Button icon="brief" onClick={() => command("open", { tab: "errand" })}>Errand <kbd>M</kbd>{s.errand && <i className="vb-active-dot"/>}</Button><Button icon="menu" aria-label="Pause" onClick={() => command("pause")}/></div>
...
function App({ store, command, overlay, geometry, controls }) {
  ...
  const titles = { domain: "Your domain", pause: "The night can wait.", interaction: s.interaction?.title || "Choose", ledger: "Night ledger", garage: "Refuge garage", intro: "Welcome to the night", result: s.result?.title || "Night report", dialogue: s.external?.speaker || "Voice in the dark", error: "Interface interrupted" };
  return <><Hud s={s} command={command}/>{s.mode && <Window title={titles[s.mode] || "ViceBlood"} description={s.mode === "domain" ? "Your city. Your connections. Your next move." : s.interaction?.detail || "World paused"} overlay={overlay} wide={s.mode === "domain"} close={() => command("close")} initialFocus={s.mode === "interaction" ? firstChoice : undefined}>
    {s.mode === "domain" && <Domain snapshot={s} geometry={geometry} command={command}/>}
    {s.mode === "interaction" && <div className="vb-choices"><p className="vb-eyebrow">E / ENTER TO CHOOSE · 1–9 SHORTCUTS</p>{s.interaction.options.map((option, i) => <button type="button" key={option.id} ref={i === s.interaction.index ? firstChoice : null} className="vb-choice" data-selected={i === s.interaction.index} disabled={option.disabled} onClick={() => command("choose", { id: option.id })}><span className="vb-choice-index">{String(i + 1).padStart(2, "0")}</span><span><strong>{option.label}</strong><small>{option.detail}</small></span><span aria-hidden="true">↗</span></button>)}</div>}
    {s.mode === "pause" && <section className="vb-section"><div className="vb-actions"><Button primary onClick={() => command("close")}>Resume night</Button><Button icon="city" onClick={() => command("open", { tab: "city" })}>Your city</Button><Button onClick={() => command("ledger")}>Police & the Veil</Button></div><h3>Accessibility</h3><Button aria-pressed={s.highContrast} onClick={() => command("contrast")}>High-contrast aim · {s.highContrast ? "On" : "Off"}</Button><p>A larger black-and-white reticle. Saved on this device.</p><details className="vb-controls"><summary>Controls</summary><pre>{controls}</pre></details></section>}
    {s.mode === "ledger" && <Ledger model={s.ledger}/>}
